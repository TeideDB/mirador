...
export default function PreviewPanel() {
  ...
  if (globalError) {
    ...
    } else if (result.format === 'pdf' && result.path) {
      const pdfUrl = `/api/files/serve?path=${encodeURIComponent(result.path)}`;
      previewContent = (
        <div className="preview-pdf">
          <div className="preview-meta">
            PDF Report — {result.pages} page{result.pages !== 1 ? 's' : ''}, {result.rows} rows, {(result.size / 1024).toFixed(1)} KB
            <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="preview-pdf-link">
              Open in new tab
            </a>
          </div>
          <iframe src={pdfUrl} className="preview-pdf-frame" title="PDF Preview" />
        </div>
      );
