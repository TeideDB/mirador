...
// Pre-read persisted state synchronously so the first render uses the correct view
// (zustand persist rehydrates async, which would flash the default view otherwise)
function readPersisted(): Partial<PipelineState> | null {
  try {
    const raw = sessionStorage.getItem('mirador-store');
    if (raw) {
...
const useStore = create<PipelineState>()(persist((set, get) => ({
  // Navigation — use pre-read persisted values so the first render matches the saved view
  currentView: (_persisted?.currentView as AppView) ?? 'workflows',
  currentProjectSlug: (_persisted?.currentProjectSlug as string | null) ?? null,
  currentWorkflowName: (_persisted?.currentWorkflowName as string | null) ?? null,
  currentDashboardName: (_persisted?.currentDashboardName as string | null) ?? null,

  setView: (view) => set({ currentView: view }),

  openWorkflow: (slug, name) => {
    const s = get();
    if (s.currentProjectSlug === slug && s.currentWorkflowName === name) {
      // Same workflow — just switch view, preserve preview/console state
      set({ currentView: 'workflow-editor' });
      return;
    }
    nodeIdCounter = 0;
    set({
  ...
  merge: (persisted, current) => {
    ...
    // Restore nodeIdCounter from rehydrated nodes
    let maxId = 0;
